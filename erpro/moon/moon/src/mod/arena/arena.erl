...
%% 角色进程:退出更新角色竞技场积分
logout_update(Role = #role{event = ?event_arena_match, cross_srv_id = <<"center">>}) ->
    arena_center:logout_update(Role);
logout_update(Role = #role{id = {RoleId, SrvId}, event = ?event_arena_match, event_pid = ArenaPid}) ->
    case is_pid(ArenaPid) andalso is_process_alive(ArenaPid) of
        true ->
...
init([ArenaLev, ArenaSeq, StartTime, RoleSize])->
    ...
    case map_mgr:create(?arena_match_map_id) of
        ...
        {ok, MapPid, MapId} ->
            ...
            State = #state{map_pid = MapPid, map_id = MapId, arena_lev = ArenaLev, arena_seq = ArenaSeq, arena_name = ArenaName, ts = erlang:now(), time = StartTime, week = CurrWeek, role_size = RoleSize},
...
handle_event({async_settlement, #arena_sp{send_13317 = Send13317, send_13304 = Send13304, role_pid = RolePid}}, match, State = #state{map_id = MapId, time = Time, role_list = RoleList, robot_alive_dragon = AliveDragon, robot_alive_tiger = AliveTiger}) ->
    case settlement(RoleList, #arena_report{robot_alive_dragon = AliveDragon, robot_alive_tiger = AliveTiger}) of
        {true, Report = #arena_report{talive = TAlive, dalive = DAlive, tiger_list = TigerList, dragon_list = DragonList}} -> %% 胜负已分
            ...
            case {Send13304, (util:unixtime() - Time) > (?arena_timeout_match_pre/1000)} of
                {?true, true} -> 
                    case TAlive > DAlive of
                        true -> 
                            send_by_arena_role_list(TigerList, {13304, {3, convert_13304(RoleList)}}),      %% 胜负已分，发送退出信息
            ...
            case (util:unixtime() - Time) > (?arena_timeout_match_pre/1000) of
                true -> 
                    erlang:send_after(?arena_timeout_stop_combat, self(), stop_all_combat),
                    {next_state, expire, State#state{report = Report}, ?arena_timeout_expire};
                false -> continue(match, State)
...
handle_sync_event({enter_match, ArenaRole = #arena_role{role_id = {RoleId, SrvId}, role_pid = RolePid, group_id = GroupId, score = Score, kill = Kill}}, _From, match, State = #state{arena_name = ArenaName, role_list = RoleList, map_id = MapId, arena_lev = ArenaLev, arena_seq = ArenaSeq, ts = Ts}) ->
    case lists:keyfind({RoleId, SrvId}, #arena_role.role_id, RoleList) of
        false ->
            ...
            role:pack_send(RolePid, 13311, {2, round((util:time_left(?arena_timeout_match_pre, Ts)  + ?arena_timeout_matching)/ 1000)}),
...
handle_sync_event({exit_match, {RoleId, SrvId}}, _From, StateName, State = #state{role_list = RoleList, arena_lev = ArenaLev, arena_seq = ArenaSeq, time = Time, week = Week}) ->
    ...
    role:apply(async, RolePid, {arena, leave_match, [ArenaRole, {Time, Week, ArenaLev, ArenaSeq}]}),
    async_settlement(self(), #arena_sp{send_13317 = ?true, send_13304 = ?true}),
...
terminate(_Reason, _StateName, State = #state{arena_lev = _ArenaLev, arena_seq = _ArenaSeq, map_id = MapId, role_list = RoleList, report = _Report = #arena_report{talive = TAlive, dalive = DAlive}}) ->
    ...
    campaign_listener:handle(arena_rank, NewRoleList, 1),
...
leave([ArenaRole = #arena_role{role_pid = RolePid, status = Status} | T], State = #state{time = Time, week = Week, arena_lev = ArenaLev, arena_seq = ArenaSeq}) ->
    case Status =:= ?arena_role_status_match orelse Status =:= ?arena_role_status_idel of
        true -> role:apply(async, RolePid, {arena, leave_match, [ArenaRole, {Time, Week, ArenaLev, ArenaSeq}]});
        false -> ignore
    end,
    leave(T, State).

%% 玩家离开竞技场
leave_match(Role = #role{id = {RoleId, SrvId}, looks = Looks, hp_max = HpMax, mp_max = MpMax, name = Name, pos = #pos{last = _LastPos}}, #arena_role{kill = Kill, kill_added = KillAdded, score = Score, group_id = GroupId, career = Career, lev = Lev, death = Death}, {Time, Week, ArenaLev, ArenaSeq}) ->
    ...
    case map:role_enter(ExitMapId, NewX, NewY, Role#role{event = ?event_no, hp = HpMax, mp = MpMax, looks = NewLooks4}) of
        {ok, NewRole} ->
            catch rank:listener(vie_acc, NewRole),
...
%% 提取角色信息
%% parse_fighter([#fighter{type = ?fighter_type_role, rid = RoleId, srv_id = SrvId} | T]) ->
%%     [{RoleId, SrvId} | parse_fighter(T)];
...
%% 战斗结束
update_role_data(State = #state{arena_lev = ArenaLev, map_id = MapId, role_list = RoleList}, {WRoleId, WSrvId, _WUseBuff}, {LRoleId, LSrvId, _LUseBuff}) ->
        ...
        WArenaRole = #arena_role{role_id = {RoleId, SrvId}, role_pid = WRolePid, kill = Kill, name = Name, group_id = WGroupId, lev = WLev, mask = WMask, score = Score, fight_capacity = WFightCapacity, series_kill = SeriesKill} ->
            case lists:keyfind({LRoleId, LSrvId}, #arena_role.role_id, RoleList) of
                LArenaRole = #arena_role{fight_capacity = LFightCapacity, name = LName, death = Death, role_pid = LRolePid, group_id = GroupId, mask = LMask, kill = LKill, score = LScore} ->
                    ...
                    AddScore = case catch role:apply(sync, WRolePid, {fun calc_buff_score/2, [OldScore]}) of
                    ...
                    role:apply(async, LRolePid, {arena, revive, [NewX, NewY, (Death + 1)]}),
...
update_role_data(State = #state{role_list = RoleList}, [{RoleId, SrvId, _UseBuff} | T]) ->
    case lists:keyfind({RoleId, SrvId}, #arena_role.role_id, RoleList) of
        ArenaRole = #arena_role{death = Death, role_pid = RolePid, group_id = GroupId} ->
            ...
            NewX = util:rand(X - 100, X + 100),
            NewY = util:rand(Y - 100, Y + 100),
            role:apply(async, RolePid, {arena, revive, [NewX, NewY, (Death + 1)]}),
...
robot_deal_winner(State = #state{arena_lev = ArenaLev, map_id = MapId, role_list = RoleList}, [#fighter{type = ?fighter_type_role, rid = RoleId, srv_id = SrvId} | T]) ->
    case lists:keyfind({RoleId, SrvId}, #arena_role.role_id, RoleList) of
...
%% 战败积分
calc_loser_score(?arena_lev_low) -> 3;
calc_loser_score(?arena_lev_middle) -> 4;
calc_loser_score(?arena_lev_hight) -> 5;
calc_loser_score(?arena_lev_super) -> 6;
calc_loser_score(?arena_lev_angle) -> 7;
calc_loser_score(_) -> 3.
                ...
                {false, false} ->
                    case lists:keyfind({TRoleId, TSrvId}, #arena_role.role_id, RoleList) of
                        ...
                        TArenaRole = #arena_role{role_pid = TRolePid, group_id = GroupId} ->
                            case is_pid(TRolePid) andalso is_process_alive(TRolePid) of
                                true ->
                                    case catch role:apply(sync, TRolePid, {arena, add_buff_to_role, []}) of
                                        ok -> 
...
send_mail_group([#arena_role{score = Score, role_id = {RoleId, SrvId}, group_id = GroupId, arena_lev = ?arena_lev_angle} | T], GroupId, Scort) ->
    NewScore = calc_mail_score(Scort, Score),
    mail:send_system({RoleId, SrvId}, {?L(<<"系统邮件">>), util:fbin(?L(<<"恭喜您所在的小组获得仙法竞技的胜利，额外获得80点仙法竞技积分，本次竞技中击败数排名第~w，获得~w个人积分和60基础积分！开服前3天击败数排名前3的玩家可以额外获得竞技场礼盒，请继续努力。">>), [Scort, NewScore]) , [{?mail_arena, NewScore + 80 + 60}], []}),
    send_mail_group(T, GroupId, Scort + 1);
send_mail_group([#arena_role{score = Score, role_id = {RoleId, SrvId}, group_id = GroupId, arena_lev = ?arena_lev_super} | T], GroupId, Scort) ->
    NewScore = calc_mail_score(Scort, Score),
    mail:send_system({RoleId, SrvId}, {?L(<<"系统邮件">>), util:fbin(?L(<<"恭喜您所在的小组获得仙法竞技的胜利，额外获得80点仙法竞技积分，本次竞技中击败数排名第~w，获得~w个人积分和60基础积分！开服前3天击败数排名前3的玩家可以额外获得竞技场礼盒，请继续努力。">>), [Scort, NewScore]) , [{?mail_arena, NewScore + 80 + 60}], []}),
    send_mail_group(T, GroupId, Scort + 1);
send_mail_group([#arena_role{score = Score, role_id = {RoleId, SrvId}, group_id = GroupId} | T], GroupId, Scort) ->
    NewScore = calc_mail_score(Scort, Score),
    mail:send_system({RoleId, SrvId}, {?L(<<"系统邮件">>), util:fbin(?L(<<"恭喜您所在的小组获得仙法竞技的胜利，额外获得40点仙法竞技积分，本次竞技中击败数排名第~w，获得~w个人积分和20基础积分！开服前3天击败数排名前3的玩家可以额外获得竞技场礼盒，请继续努力。">>), [Scort, NewScore]) , [{?mail_arena, NewScore + 20 + 40}], []}),
    send_mail_group(T, GroupId, Scort + 1);
send_mail_group([#arena_role{role_id = {RoleId, SrvId}, score = Score, arena_lev = ArenaLev} | T], GroupId, Scort) ->
    ...
    case Score > 0 of
        true ->
            NewScore = calc_mail_score(Scort, Score),
            mail:send_system({RoleId, SrvId}, {?L(<<"系统邮件">>), util:fbin(?L(<<"您在本次仙法竞技中击败数排名第~w，共获得~w个人积分和~w基础积分！开服前3天击败数排名前3的玩家可以额外获得竞技场礼盒，请继续努力。">>), [Scort, NewScore, BaseScore]), [{?mail_arena, NewScore + BaseScore}], []});
        false ->
            mail:send_system({RoleId, SrvId}, {?L(<<"系统邮件">>), util:fbin(?L(<<"您在本次仙法竞技中击败数排名第~w，获得~w基础积分！开服前3天击败数排名前3的玩家可以额外获得竞技场礼盒，请继续努力。">>), [Scort, BaseScore]), [{?mail_arena, Score + BaseScore}], []})
    end,
    send_mail_group(T, GroupId, Scort + 1).
...
award_champion([ArenaRole = #arena_role{kill =  Kill, role_id = {RoleId, SrvId}, name = Name} | T], #state{arena_name = ArenaName, arena_lev = ArenaLev, arena_seq = _ArenaSeq, time = Time}) ->
    case Kill =/= 0 of
        true ->
            arena_mgr:update_last_win(ArenaLev, RoleId, SrvId, Name, Kill),
            arena_dao_log:update_winner(RoleId, SrvId, Time, ?true),
            RoleName = util:fbin(<<"{role, ~w, ~s, ~s, #3ad6f0}">>, [RoleId, SrvId, Name]),
            notice:send(53, util:fbin(?L(<<"~s仙法竞技结束，玩家~s技压群雄，夺得天下第一之名，受众人膜拜。">>), [ArenaName, RoleName])),
...
award_tanhua([#arena_role{role_id = {RoleId, SrvId}, kill = Kill, arena_lev = ArenaLev} | T], Order, LastKill) ->
    ...
    case Order =< 3 andalso Kill =/= 0 of
        true ->
            ...
            mail:send_system({RoleId, SrvId}, {?L(<<"系统邮件">>), util:fbin(?L(<<"您在仙法竞技中仙力超群，获得仙法竞技第~w名，在开服前3天您额外获得竞技场礼盒！">>), [Order]), [], [Item]}),
            award_tanhua(T, Order + 1, Kill);
        false ->
            case Kill =:= LastKill andalso Kill =/= 0 of
                true ->
                    {ok, [Item]} = item:make(BoxId, 1, 1),
                    mail:send_system({RoleId, SrvId}, {?L(<<"系统邮件">>), ?L(<<"您在仙法竞技中仙力超群，获得仙法竞技第3名，在开服前3天您额外获得竞技场礼盒！">>), [], [Item]}),
                    award_tanhua(T, Order + 1, Kill);
                false ->
                    ignore
            end
...
stop_all_combat([#arena_role{role_pid = RolePid, name = _Name, status = ?arena_role_status_match} | T], CombatPidList) ->
    case role:apply(sync, RolePid, {arena, apply_get_combat_pid, []}) of
        {ok, CombatPid} ->
...
%% 查找积分buff
calc_buff_score(_Role = #role{id = {_RoleId, SrvId}, buff = #rbuff{buff_list = BuffList}}, OldScore) ->
    Now = util:unixtime(),
    ...
    case buff:search(BuffList, ?arena_buff_score) of
        [] -> 
            {ok, {ok, round(OldScore * Ratio)}};
        _ -> 
            {ok, {ok, round(OldScore * 1.5 * Ratio)}}
    end.
