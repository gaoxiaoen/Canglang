...
%%----------------------------------------------------
%% 登陆
login(Role = #role{event = ?event_arena_prepare, cross_srv_id = <<"center">>}) ->
    arena_center_mgr:login(Role);
login(Role = #role{id = {RoleId, SrvId}, event = ?event_arena_prepare, pos = Pos, pid = Pid}) ->
    case gen_fsm:sync_send_all_state_event(?MODULE, {login, prepare, RoleId, SrvId, Pid}) of
        {ok, _Time} -> 
            Role;
        _Any -> 
         ...
         Role#role{pos = Pos#pos{map = MapId, x = NewX, y = NewY}, event = ?event_no}
    end;
login(Role = #role{event = ?event_arena_match, cross_srv_id = <<"center">>}) ->
    arena_center_mgr:login(Role);
login(Role = #role{id = {RoleId, SrvId}, pid = Pid, event = ?event_arena_match, pos = Pos, looks = Looks}) ->
    {MapId, X, Y} = ?arena_exit,
    NewX = util:rand(X - 50, X + 50),
    NewY = util:rand(Y - 50, Y + 50),
    case gen_fsm:sync_send_all_state_event(?MODULE, {login, match, RoleId, SrvId, Pid}) of
...
%% 下线
logout(Role = #role{event = ?event_arena_prepare, cross_srv_id = <<"center">>}) ->
    arena_center_mgr:logout(Role);
logout(_Role = #role{id = {RoleId, SrvId}, event = ?event_arena_prepare}) ->
    gen_fsm:send_all_state_event(?MODULE, {logout, RoleId, SrvId});
logout(Role = #role{event = ?event_arena_match, cross_srv_id = <<"center">>}) ->
    arena_center_mgr:logout(Role);
...
%% 获取当前竞技场状态
expend_arena_pre(#role{lev = Lev}) ->
    gen_fsm:sync_send_all_state_event(?MODULE, {expend_arena_pre, Lev}).

%% @spec enter_arena(Role, Mask) -> ok | {false, Reason}
%% @doc 进入战场
enter_match(_Role = #role{id = {RoleId, SrvId}, event = ?event_arena_prepare}, Mask) ->
    case gen_fsm:sync_send_all_state_event(?MODULE, {enter_match, {RoleId, SrvId}, Mask}) of
        {ok} -> %% 好像有点多余
            {ok};
        {false, Reason} ->
            {false, Reason}
...
exit_match(#role{id = {RoleId, SrvId}, event = ?event_arena_match, event_pid = EventPid}) ->
    case is_pid(EventPid) andalso is_process_alive(EventPid) of
        false ->
            {false, ?L(<<"竞技场已经关闭">>)};
        true ->
            case arena:exit_match(EventPid, {RoleId, SrvId}) of
                {false, Reason} ->
                    {false, Reason};
                _Any ->
                    ok
...
exit_match(#role{id = {RoleId, SrvId}, event = ?event_arena_prepare})->
    case gen_fsm:sync_send_all_state_event(?MODULE, {exit_prepare, {RoleId, SrvId}}) of
        ok ->
            ok;
        {false, Reason} ->
            {false, Reason}
...
update_last_win(ArenaLev, RoleId, SrvId, Name, KillNum) ->
    gen_fsm:send_all_state_event(?MODULE, {update_last_win, ArenaLev, RoleId, SrvId, Name, KillNum}).
...
handle_event({logout, RoleId, SrvId}, StateName, State) ->
    case get_arena_role({RoleId, SrvId}, State) of
        ...
        ArenaRole = #arena_role{status = ?arena_role_status_match, arena_pid = ArenaPid} ->
            catch arena:logout(ArenaPid, RoleId, SrvId),
            NewState = update_arena_role(ArenaRole#arena_role{status = ?arena_role_status_logout}, State),
            continue(StateName, NewState);
        _ ->
            continue(StateName, State)
...
handle_sync_event({time, prepare}, _From, prepare, State = #state{ts = Ts}) ->
    Reply = round(util:time_left(?arena_timeout_prepare, Ts) / 1000),
...
handle_sync_event({time, match}, _From, match_pre, State = #state{ts = Ts}) ->
    Reply = round((util:time_left(?arena_timeout_match_pre, Ts) + ?arena_timeout_matching) / 1000),
...
handle_sync_event({enter_prepare, ArenaRole = #arena_role{arena_lev = ArenaLev, role_pid = RolePid}}, _From, prepare, State = #state{low = Low, middle = Middle, hight = Hight, super = Super, angle = Angle, ts = Ts}) ->
    role:pack_send(RolePid, 13311, {1, round(util:time_left(?arena_timeout_prepare, Ts)/ 1000)}),
    case ArenaLev of
...
handle_sync_event({exit_prepare, {RoleId, SrvId}}, _From, StateName, State) ->
    case get_arena_role({RoleId, SrvId}, State) of
        ...
        ArenaRole = #arena_role{role_pid = RolePid} ->
            role:apply(async, RolePid, {arena_mgr, leave_prepare, []}),
            NewState = delete_arena_role(ArenaRole, State),
...
handle_sync_event({enter_match, {RoleId, SrvId}, Mask}, _From, match_pre, State) ->
    case get_arena_role({RoleId, SrvId}, State) of
        ...
        ArenaRole = #arena_role{role_pid = RolePid} ->
            ...
            role:apply(async, RolePid, {arena_mgr, do_enter_match, [NewArenaRole]}),
            %% role:apply(async, RolePid, {fun do_enter_match/2, [NewArenaRole]}),
...
handle_sync_event({login, prepare, RoleId, SrvId, Pid}, _From, prepare, State = #state{ts = Ts}) ->
    case get_arena_role({RoleId, SrvId}, State) of
        ...
        ArenaRole = #arena_role{} ->
            Time = round(util:time_left(?arena_timeout_prepare, Ts)/ 1000),
    ...
    case get_arena_role({RoleId, SrvId}, State) of
        _ArenaRole = #arena_role{arena_pid = ArenaPid} when is_pid(ArenaPid) ->
            case is_process_alive(ArenaPid) of
                true ->
                    case catch arena:login(ArenaPid, RoleId, SrvId, Pid) of
                        {ok, ArenaRole2} ->
...
notice(timeout, State) ->
    ...
    case create_map(prepare, State, [?arena_lev_low, ?arena_lev_middle, ?arena_lev_hight, ?arena_lev_super, ?arena_lev_angle]) of
        ...
        NewState ->
            ...
            {next_state, prepare, NewState#state{ts = erlang:now(), timeout = ?arena_timeout_prepare}, ?arena_timeout_prepare}
    end;
notice(_Any, State) ->
    continue(notice, State).
...
prepare(timeout, State) ->
    ...
    case startup_branch(State2, Time, [?arena_lev_low, ?arena_lev_middle, ?arena_lev_hight, ?arena_lev_super, ?arena_lev_angle]) of
        {ok, NewState} ->
            {next_state, match_pre, NewState#state{ts = erlang:now(), timeout = ?arena_timeout_match_pre}, ?arena_timeout_match_pre};
        {false, Reason} ->
...
%% 报名
do_sign_up(Role = #role{ride = Ride}) ->
    case expend_arena_pre(Role) of
        {StateName, PreSeq, MapId, MapPid} ->
            case StateName of
                prepare ->
                    ArenaRole = convert(init, PreSeq, MapId, MapPid, Role),
                    case gen_fsm:sync_send_all_state_event(?MODULE, {enter_prepare, ArenaRole#arena_role{status = ?arena_role_status_prepare}}) of
                        {ok} ->
                            ...
                            case map:role_enter(MapId, NewX, NewY, Role2#role{event = ?event_arena_prepare}) of
...
start_link_arena(ArenaZone = #arena_zone{arena_lev = ArenaLev, arena_list = ArenaList, role_list = ZoneRoleList}, [RoleList | RT], [ArenaSeq | AT], Time) ->
    case arena:start_link(ArenaLev, ArenaSeq, Time, length(RoleList)) of
        {ok, Pid} ->
...
check_after_pre([#arena_online{arena_pid = ArenaPid} | T]) ->
    arena:check_after_pre(ArenaPid),
    check_after_pre(T);
check_after_pre([]) ->
    ok.
...
leave_prepare(Role = #role{name = _Name, looks = Looks, pos = #pos{last = _LastPos}, event = _Event}) ->
    ...
    case map:role_enter(ExitMapId, NewX, NewY, Role#role{event = ?event_no, looks = NewLooks}) of
        {ok, NewRole} ->
            {ok, NewRole};
        {false, _Why} ->
...
do_enter_match(Role = #role{looks = Looks, max_fc = #max_fc{max = FightCapacity}}, ArenaRole = #arena_role{arena_pid = ArenaPid, group_id = GroupId}) ->
    ...
    case arena:enter_match(ArenaPid, NewArenaRole) of
        {ok, MapId} ->
            {X, Y} = case GroupId of
            ...
            case map:role_enter(MapId, NewX, NewY, Role#role{event = ?event_arena_match, looks = GroupLooks, event_pid = ArenaPid}) of
                {ok, NewRole} ->
