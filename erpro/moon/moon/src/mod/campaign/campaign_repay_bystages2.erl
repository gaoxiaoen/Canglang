...
%% @end
%%----------------------------------------------------
-module(campaign_repay_bystages2).
-behaviour(gen_fsm).
-export(
    [
        start_link/0
...
-define(camp_repay_bystages_start, util:datetime_to_seconds({{2013, 2, 28}, {13, 00, 1}})).      %% 活动开始时间 
-define(camp_repay_bystages_end, util:datetime_to_seconds({{2013, 2, 28}, {23, 59, 59}})).      %% 活动结束时间
-define(camp_repay_bystages_second, util:datetime_to_seconds({{2013, 3, 1}, {17, 0, 0}})).     %% 第二天发放时间
-define(camp_repay_bystages_goldsum, 500). %% 每次活动都不一样
...
init([])->
    ...
    ets:new(ets_camp_repay_bystages_role2, [set, named_table, public, {keypos, #camp_bystages_role.id}]),
    ets:new(ets_camp_repay_bystages_enough2, [set, named_table, public, {keypos, #camp_bystages_role.id}]),
...
handle_event({charge, {RoleId, SrvId}, Gold}, activity, State) ->
    Key = {RoleId, SrvId},
    case ets:lookup(ets_camp_repay_bystages_enough2, Key) of
        [] ->
            case ets:lookup(ets_camp_repay_bystages_role2, Key) of
                [] ->
                    GRole = #camp_bystages_role{id = {RoleId, SrvId}, charge_sum = Gold},
                    ets:insert(ets_camp_repay_bystages_role2, GRole),
                    case Gold >= ?camp_repay_bystages_goldsum of
                        true -> 
                            send_mail(RoleId, SrvId),
                            ets:insert(ets_camp_repay_bystages_enough2, GRole);
                        _ -> ignore
                    end;
...
date_2(timeout, State) ->
    ?DEBUG("~w状态结束", [date_2]),
    List = ets:tab2list(ets_camp_repay_bystages_enough2),
    do_send_mail(List),
    Timeout = 86400 * 1000,
...
init_ets_role([[RoleId, SrvId, Gold] | T]) ->
    ...
    ets:insert(ets_camp_repay_bystages_role2, GRole),
    case Gold >= ?camp_repay_bystages_goldsum of
        true -> ets:insert(ets_camp_repay_bystages_enough2, GRole);
        _ -> ignore
    end,
    init_ets_role(T).
...
send_mail(RoleId, SrvId) ->
    Subject = ?L(<<"月末好礼100%，充值惊喜限半天">>),
    Content = util:fbin(?L(<<"亲爱的玩家您好，感谢您参与月末好礼100%活动，您在2月28日13:00至2月28日23:59充值达到500晶钻，获得500非绑定晶钻的额外返还（分5天返还），现给您返还100晶钻，祝您游戏愉快！">>), []),
