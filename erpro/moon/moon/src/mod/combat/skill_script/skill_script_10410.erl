%%----------------------------------------------------
%%
%% 当自身气血小于[HP百分比]时伤害增加[基础值+攻击*攻击转换系数]%，若自身气血不低于[气血百分比]%，则自身气血损耗[气血损耗比]%    
%%
%% args = [HP百分比，基础值，攻击转换系数，气血百分比，气血损耗比]
%% @author qingxuan
%%----------------------------------------------------
-module(skill_script_10410).
-export([
    handle_active/3
]).
-include("common.hrl").
...
handle_active(Skill = #c_skill{args = [ShpRatio1, BaseVal, DmgRatio, ShpRatio2, HpCostRatio], other_cost = OtherCost}, Self = #fighter{attr = #attr{dmg_min = DmgMin, dmg_max = DmgMax}, hp = Shp, hp_max = ShpMax}, Target) ->
    Avg = (DmgMin + DmgMax)/2,
