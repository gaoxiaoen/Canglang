%%----------------------------------------------------
%%
%% 当目标气血小于[HP百分比]时伤害增加[基础值+攻击*攻击转换系数]%，若自身气血不低于[气血百分比]%，则自身气血损耗[气血损耗值]          
%%
%% args = [HP百分比，基础值，攻击转换系数，气血百分比，气血损耗值]
%% @author qingxuan
%%----------------------------------------------------
-module(skill_script_10440).
-export([
    handle_active/3
]).
-include("common.hrl").
...
handle_active(Skill = #c_skill{args = [ThpRatio, BaseVal, DmgRatio, ShpRatio, HpCost], other_cost = OtherCost}, Self = #fighter{hp_max = ShpMax, hp = Shp, attr = #attr{dmg_min = DmgMin, dmg_max = DmgMax}}, Target = #fighter{hp_max = ThpMax, hp = Thp}) ->
    Avg = (DmgMin + DmgMax) /2,
