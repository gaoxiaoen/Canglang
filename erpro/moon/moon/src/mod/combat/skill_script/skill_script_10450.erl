%%----------------------------------------------------
%%
%% 攻击伤害提高[攻击*攻击转换系数]%，若自身气血不低于[气血百分比]%，则自身气血损耗[气血损耗值]          
%%
%% args = [攻击转换系数，气血百分比，气血损耗值]
%% @author qingxuan
%%----------------------------------------------------
-module(skill_script_10450).
-export([
    handle_active/3
]).
-include("common.hrl").
...
handle_active(Skill = #c_skill{args = [DmgRatio, HpRatio, HpCost], other_cost = OtherCost, dmg_ratio = SkDmgRatio}, Self = #fighter{hp_max = ShpMax, hp = Shp, attr = #attr{dmg_min = DmgMin, dmg_max = DmgMax}}, Target) ->
    Avg = (DmgMin + DmgMax)/2,
