...
-export([info/2, center_info/2, call/2, center_call/2]).
...
info({Lid, Aid}, Msg) ->
    case center:is_cross_center() of
        false -> center:cast(?MODULE, center_info, [{Lid, Aid}, Msg]);
        true -> center_info({Lid, Aid}, Msg)
    end;
info(_Id, _Msg) ->
    ?ERR("收到无效的飞仙历练进程消息[~w][~w]", [_Id, _Msg]),
    ok.

%% @spec center_info(Fid, Msg) -> ok
%% Fid = pid() | {integer(), integer()}
%% @doc 异步消息
center_info(Pid, Msg) when is_pid(Pid) ->
    Pid ! Msg,
    ok;
center_info({Lid, Aid}, Msg) ->
    case ets:lookup(train_pid_id_mapping, {Lid, Aid}) of
        [{_Fid, Pid}] -> Pid ! Msg;
        _ -> ok
    end;
center_info(_Fid, _Msg) ->
    ?ERR("收到无效的飞仙历练进程消息[~w][~w]", [_Fid, _Msg]),
    ok.

%% @spec call(Fid, Msg) -> {error, Reason} | term()
%% Msg = term()
%% @doc 向管理进程发送一个 handle_call 消息
call(Fid, Msg) ->
    case center:is_cross_center() of
        false -> center:call(?MODULE, center_call, [Fid, Msg]);
        true -> center_call(Fid, Msg)
    end.

%% @spec center_call(Fid, Msg) -> term() | {error, Reason}
%% @doc 向历练管理进程发送一个同步请求
center_call(Fid, Msg) ->
    TrainPid = case ets:lookup(train_pid_id_mapping, Fid) of
        [{_Fid, Pid}] -> Pid;
        _ -> Fid
    end,
    case is_pid(TrainPid) of
        true ->
            try gen_server:call(TrainPid, Msg) of
                Reply -> Reply 
            catch
                exit:{timeout, _} -> 
                    ?ERR("向飞仙历练 ~w 发起的请求{~w}发生timeout", [Fid, Msg]),
                    {error, timeout};
                exit:{noproc, _} -> 
                    ?ERR("向飞仙历练 ~w 发起的请求{~w}发生noproc", [Fid, Msg]),
                    {error, noproc};
                Error:Info ->
                    ?ERR("向飞仙历练 ~w 发起的请求{~w}发生异常{~w:~w}", [Fid, Msg, Error, Info]),
                    {error, Error}
            end;
        false ->
            {error, fid_not_found}
...
start_link(TF = #train_field{id = {Lid, Aid}}) ->
    gen_server:start_link({local, train_common:gen_train_process_name(Lid, Aid)}, ?MODULE, [TF], []).
...
handle_outer_call({ok}, State) ->
    {noreply, State};
handle_outer_call({ok, NewState}, _State) when is_record(NewState, train_field) ->
    {noreply, NewState};
handle_outer_call({reply, Reply}, State) ->
    {reply, Reply, State};
handle_outer_call({reply, Reply, NewState}, _State) when is_record(NewState, train_field) ->
