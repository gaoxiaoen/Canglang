...
%% --------------------------------------------------------------------
-module(train_mgr_center).
-behavior(gen_server).
-export([start_link/0]).
-export([init/1, handle_call/3, handle_cast/2, handle_info/2, terminate/2, code_change/3]).
-export([cast/1, call/1, center_call/1]).
...
cast(Msg) ->
    case center:is_cross_center() of
        true -> gen_server:cast(?MODULE, Msg);
        false -> center:cast(gen_server, cast, [?MODULE, Msg])
    end.

%% @spec call(Msg) -> {false, Reason} | term()
%% @doc 向历练管理进程发送一个 同步消息
call(Msg) ->
    case center:is_cross_center() of
        true -> center_call(Msg);
        false -> center:call(?MODULE, center_call, [Msg])
    end.

%% @spec center_call(Msg) -> {error, Reason} | term()
%% @doc 中央服上发起call
center_call(Msg) ->
