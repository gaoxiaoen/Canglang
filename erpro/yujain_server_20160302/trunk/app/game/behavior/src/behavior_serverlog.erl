%%%----------------------------------------------------------------------
%%% File    :behavior_serverlog.erl
%%%----------------------------------------------------------------------
-module(behavior_serverlog).
...
write_event(Fd, {Time, {error, _GL, {_Pid, Format, Args}}}) ->
    [L] = io_lib:format("~ts", [?_LANG_SERVER_LOG_ERROR]),
...
write_event(Fd, {Time, {info_msg, _GL, {_Pid, Format, Args}}}) ->
    [L] = io_lib:format("~ts", [?_LANG_SERVER_LOG_INFO]),
    do_write(Fd, Time, L, Format, Args);
...
make_log_file() ->
    ...
    io_lib:format("~s/~s_all_~p_~p_~p___~p.log", [LogDir,GameName,Year, Month, Day, Hour]).
