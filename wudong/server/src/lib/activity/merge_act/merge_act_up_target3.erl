...
init(#player{key = Pkey} = Player) ->
    StMergeActUpTarget =
        case player_util:is_new_role(Player) of
            true -> #st_merge_act_up_target3{pkey = Pkey};
            false -> activity_load:dbget_merge_act_up_target3(Pkey)
        end,
    lib_dict:put(?PROC_STATUS_MERGE_ACT_UP_TARGET3, StMergeActUpTarget),
    update_merge_act_up_target(),
    Player.

update_merge_act_up_target() ->
    StMergeActUpTarget = lib_dict:get(?PROC_STATUS_MERGE_ACT_UP_TARGET3),
    ...
    case get_act() of
        [] ->
            NewStMergeActUpTarget = #st_merge_act_up_target3{pkey = Pkey};
        #base_merge_up_target{act_id = BaseActId} ->
            Now = util:unixtime(),
            if
                BaseActId =/= ActId ->
                    NewStMergeActUpTarget = #st_merge_act_up_target3{pkey = Pkey, act_id = BaseActId, op_time = Now};
                true ->
                    NewStMergeActUpTarget = StMergeActUpTarget
            end
    end,
    lib_dict:put(?PROC_STATUS_MERGE_ACT_UP_TARGET3, NewStMergeActUpTarget).
                        ...
                        [{_ActType, _Args, RewardList} | _] ->
                            ...
                            F1 = fun({GoodId, GoodsNum, ExpireTime}) ->
                                case ExpireTime == 0 of
                                    true ->
                                        goods:make_give_goods_list(654, [{GoodId, GoodsNum}]);
                                    false ->
                                        goods:make_give_goods_list(654, [{GoodId, GoodsNum, 0, ExpireTime * ?ONE_HOUR_SECONDS + Now}])
                                end
                            end,
                            GiveGoodsList = lists:flatmap(F1, RewardList),
                            {ok, NewPlayer} = goods:give_goods(Player, GiveGoodsList),
                            NewStMergeActUpTarget = StMergeActUpTarget#st_merge_act_up_target3{recv_list = [{ActType, Args} | RecvList]},
                            lib_dict:put(?PROC_STATUS_MERGE_ACT_UP_TARGET3, NewStMergeActUpTarget),
                            activity_load:dbup_merge_act_up_target3(NewStMergeActUpTarget),
...
get_act() ->
    case activity:get_work_list(data_merge_up_target3) of
        [] -> [];
        [Base | _] -> Base
    end.
