...
init(#player{key = Pkey} = Player) ->
    StOpenActAllRank =
        case player_util:is_new_role(Player) of
            ...
            false ->
                activity_load:dbget_open_act_all_rank2(Pkey)
        end,
    lib_dict:put(?PROC_STATUS_OPEN_ALL_RANK2, StOpenActAllRank),
    update_open_act_all_rank(),
    Player.

update_open_act_all_rank() ->
    StOpenActAllRank = lib_dict:get(?PROC_STATUS_OPEN_ALL_RANK2),
    ...
    case get_act() of
        [] ->
            NewStOpenActAllRank = #st_open_act_all_rank2{pkey = Pkey};
        #base_open_act_all_rank{act_id = BaseActId} ->
            Now = util:unixtime(),
            Flag = util:is_same_date(Now, OpTime),
            if
                BaseActId =/= ActId ->
                    NewStOpenActAllRank = #st_open_act_all_rank2{pkey = Pkey, act_id = BaseActId, op_time = Now};
                Flag == false ->
                    NewStOpenActAllRank = #st_open_act_all_rank2{pkey = Pkey, act_id = BaseActId, op_time = Now};
                true ->
    ...
    lib_dict:put(?PROC_STATUS_OPEN_ALL_RANK2, NewStOpenActAllRank).
...
get_state(Player) ->
    update_open_act_all_rank(),
    StOpenActAllRank = lib_dict:get(?PROC_STATUS_OPEN_ALL_RANK2),
    #st_open_act_all_rank2{recv_list = RecvList} = StOpenActAllRank,
    case get_act() of
        [] ->
            -1;
        ...
        #base_open_act_all_rank{list = BaseList, act_id = BaseActId} ->
            ...
            if
                ...
                true ->
                    ...
                    NewStOpenActAllRank = StOpenActAllRank#st_open_act_all_rank2{recv_list = NewRecvList, op_time = util:unixtime(), act_id = BaseActId},
                    lib_dict:put(?PROC_STATUS_OPEN_ALL_RANK2, NewStOpenActAllRank),
                    activity_load:dbup_open_act_all_rank2(NewStOpenActAllRank),
                    GiveGoodsList = goods:make_give_goods_list(669,[{GiftId,1}]),
...
get_act() ->
    case activity:get_work_list(data_open_all_rank2) of
        [] -> [];
        [Base | _] -> Base
    end.
