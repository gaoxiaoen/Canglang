...
init_ets() ->
    ets:new(?ETS_OPEN_ALL_TARGET2, [{keypos, #ets_open_all_target.key} | ?ETS_OPTIONS]).
...
init() ->
    case get_act() of
        ...
        #base_open_all_target{act_id = BaseActId, list = BaseList} ->
            DbList = activity_load:dbget_all_open_act_all_target2(BaseActId),
...
init(#player{key = Pkey} = Player) ->
    StOpenActAllTarget =
        case player_util:is_new_role(Player) of
            true -> #st_open_act_all_target2{pkey = Pkey};
            false -> activity_load:dbget_open_act_all_target2(Pkey)
        end,
    lib_dict:put(?PROC_STATUS_OPEN_ALL_TARGET2, StOpenActAllTarget),
...
init_login(#player{key = Pkey}) ->
    case get_act() of
        ...
        _ ->
            F = fun(Type) ->
                Lv = get_stage(Type),
                ?CAST(activity_proc:get_act_pid(), {update_open_act_all_target2, Pkey, Type, Lv})
            end,
...
update_open_act_all_target() ->
    StOpenActAllTarget = lib_dict:get(?PROC_STATUS_OPEN_ALL_TARGET2),
    ...
    case get_act() of
        [] ->
            NewStOpenActAllTarget = #st_open_act_all_target2{pkey = Pkey};
        #base_open_all_target{act_id = BaseActId} ->
            Now = util:unixtime(),
            if
                BaseActId =/= ActId ->
                    NewStOpenActAllTarget = #st_open_act_all_target2{pkey = Pkey, act_id = BaseActId, op_time = Now};
                true ->
                    NewStOpenActAllTarget = StOpenActAllTarget
            end
    end,
    lib_dict:put(?PROC_STATUS_OPEN_ALL_TARGET2, NewStOpenActAllTarget).
...
get_act_info(_Player) ->
    update_open_act_all_target(),
    StOpenActAllTarget = lib_dict:get(?PROC_STATUS_OPEN_ALL_TARGET2),
    #st_open_act_all_target2{recv_list = RecvList} = StOpenActAllTarget,
    case get_act() of
        [] ->
            {0, 1, []};
        ...
        #base_open_all_target{list = BaseList, act_id = BaseActId} ->
            ...
            if
                ...
                true ->
                    ...
                    GiveGoodsList = goods:make_give_goods_list(671, [{GiftId, 1}]),
                    {ok, NewPlayer} = goods:give_goods(Player, GiveGoodsList),
                    NewStOpenActAllTarget =
                        StOpenActAllTarget#st_open_act_all_target2{
                            recv_list = [{ActType, BaseLv, BaseNum} | RecvList]
                        },
                    lib_dict:put(?PROC_STATUS_OPEN_ALL_TARGET2, NewStOpenActAllTarget),
                    activity_load:dbup_open_act_all_target2(NewStOpenActAllTarget),
...
act_target_init() ->
    ?CAST(activity_proc:get_act_pid(), act_target_init2).
...
act_target(Pkey, ActType, Lv) ->
    ?CAST(activity_proc:get_act_pid(), {update_open_act_all_target2, Pkey, ActType, Lv}).
...
act_target(Pkey, BaseActId, ActType, BaseLv, BaseNum, IsDb) ->
    ...
    case lists:keyfind(Pkey, 1, PlayerList) of
        false ->
            ...
            ets:insert(?ETS_OPEN_ALL_TARGET2, NewEts),
            ?IF_ELSE(IsDb == true, activity_load:dbup_open_act_all_target_ets2(NewEts), skip);
        _ -> %% 避免重复计算玩家数量
            skip
    end.
...
sys_midnight_refresh() ->
    case get_act() of
        [] ->
            ets:delete_all_objects(?ETS_OPEN_ALL_TARGET2);
        #base_open_all_target{act_id = BaseActId} ->
            List = ets:tab2list(?ETS_OPEN_ALL_TARGET2),
            if
                List == [] -> skip;
                true ->
                    Ets = hd(List),
                    if
                        ...
                        true -> ets:delete_all_objects(?ETS_OPEN_ALL_TARGET2)
                    end
            end,
            %% 重新load数据
            act_target_init()
...
lookup(BaseActId, ActType, BaseLv, BaseNum) ->
    Key = {BaseActId, ActType, BaseLv, BaseNum},
    case ets:lookup(?ETS_OPEN_ALL_TARGET2, Key) of
        [] ->
            Ets =
                #ets_open_all_target{
                    act_id = BaseActId,
...
get_act() ->
    case activity:get_work_list(data_open_all_target2) of
        [] -> [];
        [Base | _] -> Base
    end.
