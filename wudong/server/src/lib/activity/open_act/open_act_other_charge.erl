...
init(#player{key = Pkey} = Player) ->
    St =
        case player_util:is_new_role(Player) of
            true -> #st_act_other_charge{pkey = Pkey};
            false -> activity_load:dbget_act_other_charge(Pkey)
        end,
    lib_dict:put(?PROC_STATUS_ACT_OTHER_CHARGE, St),
    update_merge_act_acc_charge(),
    Player.

update_merge_act_acc_charge() ->
    StMergeActAccCharge = lib_dict:get(?PROC_STATUS_ACT_OTHER_CHARGE),
    #st_act_other_charge{
        pkey = Pkey,
        act_id = ActId
    } = StMergeActAccCharge,
    case get_act() of
        [] ->
            NewStMergeActAccCharge = #st_act_other_charge{pkey = Pkey};
        #base_act_other_charge{act_id = BaseActId} ->
            Now = util:unixtime(),
            if
                BaseActId =/= ActId ->
                    NewStMergeActAccCharge = #st_act_other_charge{pkey = Pkey, act_id = BaseActId, op_time = Now};
                true ->
                    NewStMergeActAccCharge = StMergeActAccCharge#st_act_other_charge{acc_charge_gold = act_charge:get_charge_gold()}
            end
    end,
    lib_dict:put(?PROC_STATUS_ACT_OTHER_CHARGE, NewStMergeActAccCharge).
...
recv(Player, Id) ->
    ...
    case get_act() of
        [] ->
            {4, Player};
        #base_act_other_charge{charge_list = BaseChargeList} ->
                    ...
                    if
                        ...
                        true ->
                            ...
                            NewStMergeActAccCharge =
                                StMergeActAccCharge#st_act_other_charge{
                                    recv_list = [Id | RecvList],
                                    op_time = util:unixtime()
                                },
                            lib_dict:put(?PROC_STATUS_ACT_OTHER_CHARGE, NewStMergeActAccCharge),
                            activity_load:dbup_act_other_charge(NewStMergeActAccCharge),
...
add_charge(_Player, ChargeGold) ->
    StMergeActAccCharge = lib_dict:get(?PROC_STATUS_ACT_OTHER_CHARGE),
    #st_act_other_charge{acc_charge_gold = AccChargeGold} = StMergeActAccCharge,
    case get_act() of
        [] ->
            skip;
        _ ->
            NewStMergeActAccCharge = StMergeActAccCharge#st_act_other_charge{acc_charge_gold = AccChargeGold + ChargeGold},
            lib_dict:put(?PROC_STATUS_ACT_OTHER_CHARGE, NewStMergeActAccCharge),
            activity_load:dbup_act_other_charge(NewStMergeActAccCharge)
    end.

get_act() ->
    case activity:get_work_list(data_act_other_charge) of
        [] -> [];
        [Base | _] -> Base
    end.