...
init(#player{key = Pkey} = Player) ->
    StOpenActUpTarget =
        case player_util:is_new_role(Player) of
            true -> #st_open_act_up_target2{pkey = Pkey};
            false -> activity_load:dbget_open_act_up_target2(Pkey)
        end,
    lib_dict:put(?PROC_STATUS_OPEN_ACT_UP_TARGET2, StOpenActUpTarget),
    update_open_act_up_target(),
    Player.

update_open_act_up_target() ->
    StOpenActUpTarget = lib_dict:get(?PROC_STATUS_OPEN_ACT_UP_TARGET2),
    ...
    if
        SysOpenDay =/= OpenDay ->
            NewStOpenActUpTarget = #st_open_act_up_target2{pkey = Pkey, open_day = SysOpenDay, op_time = util:unixtime()};
        true ->
            NewStOpenActUpTarget = StOpenActUpTarget
    end,
    lib_dict:put(?PROC_STATUS_OPEN_ACT_UP_TARGET2, NewStOpenActUpTarget).
                        ...
                        [{_BaseType, _Args, RewardList} | _] ->
                            ...
                            F1 = fun({GoodId, GoodsNum, ExpireTime}) ->
                                case ExpireTime == 0 of
                                    true ->
                                        goods:make_give_goods_list(667, [{GoodId, GoodsNum}]);
                                    false ->
                                        goods:make_give_goods_list(667, [{GoodId, GoodsNum, 0, ExpireTime * ?ONE_HOUR_SECONDS + Now}])
                                end
                            end,
                            GiveGoodsList = lists:flatmap(F1, RewardList),
                            {ok, NewPlayer} = goods:give_goods(Player, GiveGoodsList),
                            NewStOpenActUpTarget = StOpenActUpTarget#st_open_act_up_target2{recv_list = [{BaseType, Args} | RecvList], op_time = util:unixtime()},
                            lib_dict:put(?PROC_STATUS_OPEN_ACT_UP_TARGET2, NewStOpenActUpTarget),
                            activity_load:dbup_open_act_up_target2(NewStOpenActUpTarget),
...
get_act(OpenDay) ->
    case ets:lookup(?ETS_ACT_OPEN_INFO, OpenDay) of
        ...
        [#ets_act_info{act_info = ActInfo}] ->
            case lists:keyfind(?ACT_UP_TARGET_TWO, 1, ActInfo) of
                false -> false;
                {_Act, ActType} ->
                    ActType
            end
