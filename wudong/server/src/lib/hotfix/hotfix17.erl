...
%% 写入文本
write_to_text(List) ->
    FileName = io_lib:format("../bug_list.txt", []),
    {ok, S} = file:open(FileName, write),
    lists:foreach(fun({ServerId, List1}) ->
        lists:foreach(fun(Other) ->
            {Pkey, Gold, Bgold, Remain, Info} = Other,
            io:format(S, ";~p ;~p ;~p; ~p; ~p; ~p; ~n", [ServerId, Pkey, Gold, Bgold, Remain, Info])
        end, List1)
    end, List),
    file:close(S).
...
back() ->
    PlayerKeyList =
        [
            {300011431, 0, 100},
            {267400172, 1516066087, 9976},
            {267400557, 1516097520, 19288},
            {267400818, 1516062316, 21068},
            {267900078, 1516190475, 25496},
            {267900524, 1516121024, 25884},
            {268000406, 1516111128, 38668},
            {268000568, 1516157025, 41404},
            {268000841, 1516111901, 140344},
            {355900043, 1516187940, 145168},
            {355900081, 1516184769, 163236},
            {355900132, 1516094104, 199708},
            {355900246, 1516093679, 239284},
            {355900577, 1516171338, 295544},
            {854900994, 1516096437, 296284},
            {855000542, 1516072606, 464324},
            {952900155, 1516121764, 983104},
            {953500082, 1516081267, 1023768}],
    List = [
        mount, wing, magic_weapon, light_weapon, pet_weapon, footprint, cat, golden_body, baby_wing, baby_weapon, baby_mount, god_treasure, jade
    ],
    F = fun({Pkey, BackTime, Gold}) ->
        Sql11 = io_lib:format("SELECT bgold FROM player_state where pkey = ~p", [Pkey]),
        case db:get_row(Sql11) of
            [Bgold] ->
                FF = fun(GoodsId) ->
                    back_to_zero_time3(GoodsId, 1, Pkey, BackTime)
                end,
                List1 = lists:flatmap(FF, List),
                [{Pkey, Gold, Bgold, Gold - Bgold, List1}];
            _ ->
                []
        end
    end,
    %% 回滚外观阶级
    lists:flatmap(F, PlayerKeyList).

%%     F3 = fun({Pkey, BackTime}) ->
%%         FF3 = fun(GoodsId) ->
...
%%     end,
%%     %% 回滚外观技能ID
%%     List2 = lists:map(F3, PlayerKeyList),
%%
%%     F4 = fun({Pkey, _BackTime}) ->
%%         FF4 = fun(GoodsId) ->
...
back_to_zero_time3(mount, _GoodsNum, _PlayerId, Time) ->
    ...
    case db:get_row(Sql) of
        [Lv, Exp] ->
            Sql2 = io_lib:format("select stage,exp FROM mount  where pkey = ~p limit 1", [_PlayerId]),
            case db:get_row(Sql2) of
                [Lv2, Exp2] ->
                    [{mount, Lv, Exp, Lv2, Exp2}];
                _ ->
                    [{mount, Lv, Exp, 0, 0}]
            end;
...
back_to_zero_time3(wing, _GoodsNum, _PlayerId, Time) ->
    ...
    case db:get_row(Sql) of
        [Lv, Exp] ->
            Sql2 = io_lib:format("select stage,exp FROM wing  where pkey = ~p limit 1", [_PlayerId]),
            case db:get_row(Sql2) of
                [Lv2, Exp2] ->
                    [{wing, Lv, Exp, Lv2, Exp2}];
                _ ->
                    [{wing, Lv, Exp, 0, 0}]
            end;
%%             ?DEBUG("Lv:~p,Exp:~p", [Lv, Exp]),
...
back_to_zero_time3(magic_weapon, _GoodsNum, _PlayerId, Time) ->
    ...
    case db:get_row(Sql) of
        [Lv, Exp] ->
            Sql2 = io_lib:format("select stage,exp FROM magic_weapon  where pkey = ~p limit 1", [_PlayerId]),
            case db:get_row(Sql2) of
                [Lv2, Exp2] ->
                    [{magic_weapon, Lv, Exp, Lv2, Exp2}];
                _ ->
                    [{magic_weapon, Lv, Exp, 0, 0}]
            end;
%%         ?DEBUG("Lv:~p,Exp:~p", [Lv, Exp]),
...
back_to_zero_time3(light_weapon, _GoodsNum, _PlayerId, Time) ->
    ...
    case db:get_row(Sql) of
        [Lv, Exp] ->
            Sql2 = io_lib:format("select stage,exp FROM light_weapon  where pkey = ~p limit 1", [_PlayerId]),
            case db:get_row(Sql2) of
                [Lv2, Exp2] ->
                    [{light_weapon, Lv, Exp, Lv2, Exp2}];
                _ ->
                    [{light_weapon, Lv, Exp, 0, 0}]
            end;
%%             ?DEBUG("Lv:~p,Exp:~p", [Lv, Exp]),
...
back_to_zero_time3(pet_weapon, _GoodsNum, _PlayerId, Time) ->
    ...
    case db:get_row(Sql) of
        [Lv, Exp] ->
            Sql2 = io_lib:format("select stage,exp FROM pet_weapon  where pkey = ~p limit 1", [_PlayerId]),
            case db:get_row(Sql2) of
                [Lv2, Exp2] ->
                    [{pet_weapon, Lv, Exp, Lv2, Exp2}];
                _ ->
                    [{pet_weapon, Lv, Exp, 0, 0}]
            end;
%%             ?DEBUG("Lv:~p,Exp:~p", [Lv, Exp]),
...
back_to_zero_time3(footprint, _GoodsNum, _PlayerId, Time) ->
    ...
    case db:get_row(Sql) of
        [Lv, Exp] ->
            Sql2 = io_lib:format("select stage,exp FROM footprint  where pkey = ~p limit 1", [_PlayerId]),
            case db:get_row(Sql2) of
                [Lv2, Exp2] ->
                    [{footprint, Lv, Exp, Lv2, Exp2}];
                _ ->
                    [{footprint, Lv, Exp, 0, 0}]
            end;
%%             ?DEBUG("Lv:~p,Exp:~p", [Lv, Exp]),
...
back_to_zero_time3(cat, _GoodsNum, _PlayerId, Time) ->
    ...
    case db:get_row(Sql) of
        [Lv, Exp] ->
            Sql2 = io_lib:format("select stage,exp FROM cat  where pkey = ~p limit 1", [_PlayerId]),
            case db:get_row(Sql2) of
                [Lv2, Exp2] ->
                    [{cat, Lv, Exp, Lv2, Exp2}];
                _ ->
...
back_to_zero_time3(golden_body, _GoodsNum, _PlayerId, Time) ->
    ...
    case db:get_row(Sql) of
        [Lv, Exp] ->
            Sql2 = io_lib:format("select stage,exp FROM golden_body  where pkey = ~p limit 1", [_PlayerId]),
            case db:get_row(Sql2) of
                [Lv2, Exp2] ->
                    [{golden_body, Lv, Exp, Lv2, Exp2}];
                _ ->
                    [{golden_body, Lv, Exp, 0, 0}]
            end;
%%
%%         ?DEBUG("Lv:~p,Exp:~p", [Lv, Exp]),
...
back_to_zero_time3(baby_wing, _GoodsNum, _PlayerId, Time) ->
    ...
    case db:get_row(Sql) of
        [Lv, Exp] ->
            Sql2 = io_lib:format("select stage,exp FROM baby_wing  where pkey = ~p limit 1", [_PlayerId]),
            case db:get_row(Sql2) of
                [Lv2, Exp2] ->
                    [{baby_wing, Lv, Exp, Lv2, Exp2}];
                _ ->
                    [{baby_wing, Lv, Exp, 0, 0}]
            end;
%%             ?DEBUG("Lv:~p,Exp:~p", [Lv, Exp]),
...
%%灵佩进阶 等级
back_to_zero_time3(jade, _GoodsNum, _PlayerId, Time) ->
    Sql = io_lib:format("SELECT old_stage,exp FROM log_jade_stage WHERE pkey = ~p AND `time` >= ~p ORDER BY `time`,`exp` limit 1", [_PlayerId, Time]),
    case db:get_row(Sql) of
        [Lv, Exp] ->
            Sql2 = io_lib:format("select stage,exp FROM jade  where pkey = ~p limit 1", [_PlayerId]),
            case db:get_row(Sql2) of
                [Lv2, Exp2] ->
                    [{jade, Lv, Exp, Lv2, Exp2}];
                _ ->
                    [{jade, Lv, Exp, 0, 0}]
            end;
%%             ?DEBUG("Lv:~p,Exp:~p", [Lv, Exp]),
%%             UpdateSql = io_lib:format("update jade set jade_id=110001, stage = ~p,exp = ~p where pkey = ~p", [Lv, Exp, _PlayerId]),
%%             db:execute(UpdateSql);
        _R ->
            []
    end;
...
%%仙宝进阶 等级
back_to_zero_time3(god_treasure, _GoodsNum, _PlayerId, Time) ->
    Sql = io_lib:format("SELECT old_stage,exp FROM log_god_treasure_stage WHERE pkey = ~p AND `time` >= ~p ORDER BY `time`,`exp` limit 1", [_PlayerId, Time]),
    case db:get_row(Sql) of
        [Lv, Exp] ->
            Sql2 = io_lib:format("select stage,exp FROM god_treasure  where pkey = ~p limit 1", [_PlayerId]),
            case db:get_row(Sql2) of
                [Lv2, Exp2] ->
                    [{god_treasure, Lv, Exp, Lv2, Exp2}];
                _ ->
                    [{god_treasure, Lv, Exp, 0, 0}]
            end;
%%             ?DEBUG("Lv:~p,Exp:~p", [Lv, Exp]),
%%             UpdateSql = io_lib:format("update god_treasure set god_treasure_id=110001, stage = ~p,exp = ~p where pkey = ~p", [Lv, Exp, _PlayerId]),
%%             db:execute(UpdateSql);
        _R ->
            []
    end;
...
%%灵骑进阶 等级
back_to_zero_time3(baby_mount, _GoodsNum, _PlayerId, Time) ->
    Sql = io_lib:format("SELECT old_stage,exp FROM log_baby_mount_stage WHERE pkey = ~p AND `time` >= ~p ORDER BY `time`,`exp` limit 1", [_PlayerId, Time]),
    case db:get_row(Sql) of
        [Lv, Exp] ->
            Sql2 = io_lib:format("select stage,exp FROM baby_mount  where pkey = ~p limit 1", [_PlayerId]),
            case db:get_row(Sql2) of
                [Lv2, Exp2] ->
                    [{baby_mount, Lv, Exp, Lv2, Exp2}];
                _ ->
                    [{baby_mount, Lv, Exp, 0, 0}]
            end;
%%             ?DEBUG("Lv:~p,Exp:~p", [Lv, Exp]),
%%             UpdateSql = io_lib:format("update baby_mount set baby_mount_id=110001, stage = ~p,exp = ~p where pkey = ~p", [Lv, Exp, _PlayerId]),
%%             db:execute(UpdateSql);
        _R ->
            []
    end;
...
%%灵骑进阶 等级
back_to_zero_time3(baby_weapon, _GoodsNum, _PlayerId, Time) ->
    Sql = io_lib:format("SELECT old_stage,exp FROM log_baby_weapon_stage WHERE pkey = ~p AND `time` >= ~p ORDER BY `time`,`exp` limit 1", [_PlayerId, Time]),
    case db:get_row(Sql) of
        [Lv, Exp] ->
            Sql2 = io_lib:format("select stage,exp FROM baby_weapon  where pkey = ~p limit 1", [_PlayerId]),
            case db:get_row(Sql2) of
                [Lv2, Exp2] ->
                    [{baby_weapon, Lv, Exp, Lv2, Exp2}];
                _ ->
                    [{baby_weapon, Lv, Exp, 0, 0}]
            end;
%%             ?DEBUG("Lv:~p,Exp:~p", [Lv, Exp]),
%%             UpdateSql = io_lib:format("update baby_weapon set current_image_id=110001, stage = ~p,exp = ~p where pkey = ~p", [Lv, Exp, _PlayerId]),
%%             db:execute(UpdateSql);
        _R ->
            []
    end.
