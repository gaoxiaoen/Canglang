...
-export([make_bug_info/0,get_bug_list/0,back_zero_time20/1,back_to_zero_time3/3]).
...
get_bug_list() ->
        ...
        case db:get_all("SELECT pkey,nickname,goods FROM log_act_lucky_turn WHERE opera = 3 AND `time` >= 1505404800 order by `time`") of
            List when is_list(List) ->
                lists:foldl(fun([Pkey, NickName,Goods], AccList) ->
                    ...
                    case get(?BUG_KEY_ID_3(Pkey)) of
                        undefined ->
                            put(?BUG_KEY_ID_3(Pkey), 1),
                            AccList;
                        _ ->
    ...
    PlayerIds = get_ids(),
    BugList2 =
        lists:foldl(fun({Key, _NickName,RfGoods},AccList) ->
            RfGoods2 = goods:merge_goods(RfGoods),
            case lists:member(Key,PlayerIds) of
                true ->
                    [{Key,RfGoods2}|AccList];
                _ ->
                    AccList
            end
                  end, [],BugList),
...
back_zero_time20(BackList) ->
    ServerNum = config:get_server_num(),
    case lists:keyfind(ServerNum,1,BackList) of
        {_,IdPlayers} ->
            lists:foreach(fun({Pkey,_}) ->
    ...
    case lists:keyfind(ServerNum,1,BackList) of
        {_,IdPlayers} ->
            lists:foreach(fun({PlayerId,RfGoods}) ->
                ?ERR("Back Zero Time2 player_id:~w,RfGoods:~w",[PlayerId,RfGoods]),
                util:sleep(200),
...
back_to_zero_time3(6602008,_GoodsNum,_PlayerId) ->
   FashionId = 10008,
   Sql = io_lib:format("SELECT stage FROM log_fashion WHERE pkey = ~w AND fashion_id = ~w AND `time` >= 1505404800 ORDER BY `time` LIMIT 1",[_PlayerId,FashionId]),
...
back_to_zero_time3(4103015,_GoodsNum,_PlayerId) ->
   Sql = io_lib:format("SELECT pet_key,star,exp FROM log_pet_star WHERE pkey = ~w AND `time` >= 1505404800  AND pet_type_id = 10015 ORDER BY `time`",[_PlayerId]),
...
%% 300012991
   Sql = io_lib:format(" SELECT pkey,goods_id,befor_lv FROM log_equip_god_forging WHERE pkey = ~p AND time > 1505404800 ORDER BY time ", [PlayerId]),
   case db:get_all(Sql) of
...
back_to_zero_time3(20340, _GoodsNum, _PlayerId) ->
   Sql = io_lib:format("SELECT stage,lv,exp FROM log_pet_stage WHERE pkey = ~w AND `time` >= 1505404800 ORDER BY `time`,`exp` limit 1", [_PlayerId]),
   case db:get_row(Sql) of
       [Stage, Lv,Exp] ->
...
back_to_zero_time3(8001054,_GoodsNum,_PlayerId) ->
   %% log_equip_upgrade
   Sql = io_lib:format("select gkey, max(after_goods_id) from log_equip_upgrade where pkey=~p and time < 1505404799 group by gkey", [_PlayerId]),
   case db:get_all(Sql) of
       Rows when is_list(Rows) ->
           F = fun([Gkey, GoodsId]) ->
...
back_to_zero_time3(2003000,_GoodsNum,_PlayerId) ->
   Sql = io_lib:format("select goods_id, max(after_lv) as lv from log_equip_streng where pkey=~p and time < 1505404799 group by goods_id", [_PlayerId]),
   case db:get_all(Sql) of
       Rows when is_list(Rows) ->
           F = fun([GoodsId, Lv]) ->
...
%% 宝石等级 // 1级宝石礼包
back_to_zero_time3(8002401,_GoodsNum,_PlayerId) ->
    ok;
