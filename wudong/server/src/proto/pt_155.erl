...
%% 通讯协议编码、解码处理
%% author by 苍狼工作室
%% createtime 2017-11-01 17:39:50
%%----------------------------------------------------
-module(pt_155).
...
%% 法器信息 
write(15501, {P0_stage, P0_exp, P0_cd, P0_figure, P0_cbp, P0_grow_num, P0_attribute_list, P0_skill_list, P0_equip_list, P0_spirit_state}) ->
    ...
    {ok, <<(byte_size(D_a_t_a) + 7):32, 15501:16,0:8, D_a_t_a/binary>>};


%% 升阶 
write(15502, {P0_code}) ->
    D_a_t_a = <<P0_code:8>>,
    {ok, <<(byte_size(D_a_t_a) + 7):32, 15502:16,0:8, D_a_t_a/binary>>};


%% 切换形象 
write(15503, {P0_code}) ->
    D_a_t_a = <<P0_code:8>>,
    {ok, <<(byte_size(D_a_t_a) + 7):32, 15503:16,0:8, D_a_t_a/binary>>};


%% 激活技能 
write(15504, {P0_ret}) ->
    D_a_t_a = <<P0_ret:8>>,
    {ok, <<(byte_size(D_a_t_a) + 7):32, 15504:16,0:8, D_a_t_a/binary>>};


%% 升级技能 
write(15505, {P0_ret}) ->
    D_a_t_a = <<P0_ret:8>>,
    {ok, <<(byte_size(D_a_t_a) + 7):32, 15505:16,0:8, D_a_t_a/binary>>};


%% 技能列表更新 
write(15506, {P0_skill_list}) ->
    ...
    {ok, <<(byte_size(D_a_t_a) + 7):32, 15506:16,0:8, D_a_t_a/binary>>};


%% 穿上装备 
write(15507, {P0_error_code}) ->
    D_a_t_a = <<P0_error_code:8>>,
    {ok, <<(byte_size(D_a_t_a) + 7):32, 15507:16,0:8, D_a_t_a/binary>>};


%% 使用法宝成长丹 
write(15508, {P0_error_code}) ->
    D_a_t_a = <<P0_error_code:8>>,
    {ok, <<(byte_size(D_a_t_a) + 7):32, 15508:16,0:8, D_a_t_a/binary>>};


%% 领取法宝 
write(15509, {P0_error_code}) ->
    D_a_t_a = <<P0_error_code:8>>,
    {ok, <<(byte_size(D_a_t_a) + 7):32, 15509:16,0:8, D_a_t_a/binary>>};


%% 灵脉信息 
write(15510, {P0_stage, P0_cur_type, P0_type_list, P0_cbp, P0_attr_list, P0_spirit}) ->
    ...
    {ok, <<(byte_size(D_a_t_a) + 7):32, 15510:16,0:8, D_a_t_a/binary>>};


%% 升级灵脉 
write(15511, {P0_code}) ->
    D_a_t_a = <<P0_code:8>>,
    {ok, <<(byte_size(D_a_t_a) + 7):32, 15511:16,0:8, D_a_t_a/binary>>};
