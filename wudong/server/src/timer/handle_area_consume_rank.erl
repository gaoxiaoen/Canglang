...
%%%-------------------------------------------------------------------
-module(handle_area_consume_rank).
-author("luobq").
...
handle(State, NowTime) ->
    ...
    if
        IsCenterAll andalso LeaveTime =< 60 andalso LeaveTime =/= 0 ->
            ...
            spawn(fun() -> util:sleep(max(LeaveTime - 1, 0) * 1000), area_consume_rank_proc:end_reward(Base) end); %% 开奖
        true ->
            if
                M rem 10 == 0 ->
                    spawn(fun() -> util:sleep(util:rand(500, 5000)), area_consume_rank_proc:cmd_refresh() end);
                true -> ok
            end
    end,
    {ok, State}.
