...
%%%-------------------------------------------------------------------
-module(handle_cross_recharge_rank).
-author("luobq").
...
handle(State, NowTime) ->
    ...
    if
        IsCenterAll andalso LeaveTime =< 60 andalso LeaveTime =/= 0 ->
            Base = cross_recharge_rank:get_act(),
            spawn(fun() -> util:sleep(max(LeaveTime-1, 0) * 1000), cross_recharge_rank_proc:end_reward(Base) end); %% 开奖

        true ->
            if
                M rem 10 == 0 ->
                    spawn(fun() -> cross_recharge_rank_proc:cmd_refresh() end);
                true -> ok
            end
    end,
    {ok, State}.
