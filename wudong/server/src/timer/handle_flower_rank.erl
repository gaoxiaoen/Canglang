...
handle(State, _NowTime) ->
    ...
    if
         LeaveTime =< 60 andalso LeaveTime =/= 0 ->
             LeaveTime0 = LeaveTime - 5,
             ?DO_IF(LeaveTime0 =< 0,?ERR("handle_flower_rank err !! LeaveTime0 : ~p~n",[LeaveTime0])),
             spawn(fun() -> util:sleep(max(0,LeaveTime0) * 1000), flower_rank_proc:end_reward() end); %% 开奖
        true ->
            ok
    end,
    {ok, State}.
