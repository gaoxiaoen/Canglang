...
%%% @Author  : zj
%%% @Created : 2013.8.6
%%% @Description: 定时器每小时执行一次 。每个节点都在执行，如只需执行一次，需在逻辑代码加上节点号限制。
%%%-----------------------------------
-module(timer_hour).
...
% 回调模块列表
% 如：-define(MODLE_LIST, [timer_guild, timer_mail, timer_drop, timer_buff, timer_guild_battle, timer_master]).
-define(MODLE_LIST, [
    handle_arena,
    handle_eliminate,
    handle_field_boss,
    handle_cross_fruit,
    handle_dungeon_guard
]).
...
init([]) ->
    case do_init(?MODLE_LIST, [], []) of
        {ok, NewModuleList, NewStateList} ->
            Timeout = (3600 - util:unixtime() rem 3600) * 1000,
