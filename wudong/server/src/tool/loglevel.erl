...
-export([set/1, get/0 ,set3/0, set6/0]).
...
logger_src(Loglevel) ->
   ...
   get() -> "++ L ++".
    ...
    test_msg(Module, Line, Format, Args) when " ++ L ++ " >= 6 ->
            io:format(\"T(~p:~p:~p) : \"++Format++\"~n\",[self(), Module, Line]++Args);
    test_msg(_,_,_,_) -> 
            %%
            ok.

    debug_msg(Module, Line, Format, Args) when " ++ L ++ " >= 5 ->
            io:format(\"D(~p:~p:~p) : \"++Format++\"~n\",[self(), Module, Line]++Args);
    debug_msg(_,_,_,_) -> 
            %%
            ok.
